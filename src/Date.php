...
class Date
{
    ...
    public static function unixtime(
        ...
        switch ($type) {
            ...
            case 'day':
                ...
                break;
            case 'week':
                $time = $position ? mktime(0, 0, 0, $month, $day - date("w", mktime(0, 0, 0, $month, $day, $year)) + 1 - 7 * (-$offset), $year) : mktime(23, 59, 59, $month, $day - date("w", mktime(0, 0, 0, $month, $day, $year)) + 7 - 7 * (-$offset), $year);
                break;
            case 'month':
                ...
                break;
            case 'quarter':
                $time = $position ? mktime(0, 0, 0, 1 + ((ceil(date('n', mktime(0, 0, 0, $month, $day, $year)) / 3) + $offset) - 1) * 3, 1, $year) : mktime(23, 59, 59, (ceil(date('n', mktime(0, 0, 0, $month, $day, $year)) / 3) + $offset) * 3, cal_days_in_month(CAL_GREGORIAN, (ceil(date('n', mktime(0, 0, 0, $month, $day, $year)) / 3) + $offset) * 3, $year), $year);
                break;
            case 'year':
    ...
    /**
     * 根据时间戳，获得时间描述，如：刚刚，xx分钟前，xx小时前，今天，昨天，前天
     *
     * @param $time 时间戳
     * @return null|string
     */
